...
#' Plot the sensitivty, specificity and FDR as we increase the amount of noise with which the proportion of cancer cells are measured.
...
#' Supplementary Figure: Do an analysis where we subset to varying amounts of the data (by 95% -> 5%)
sensitivitys <- numeric()
specificitys <- numeric()
trueFdrs <- numeric()
for(j in 1:19)
{
  print(j)
  samplesWithMostCancerCells <- which(rank(propInvNoise) < (j*50))
  top10pctEffectConventaionalModel <- numeric()
  top10pctPVALConventaionalModel <- numeric()
  for(i in 1:nrow(bulkExpressionSimMat))
  {
    top10pctEffectConventaionalModel[i] <- coef(summary(lm(bulkExpressionSimMat[i,samplesWithMostCancerCells]~genotype[samplesWithMostCancerCells])))[2, 1]
    top10pctPVALConventaionalModel[i] <- coef(summary(lm(bulkExpressionSimMat[i,samplesWithMostCancerCells]~genotype[samplesWithMostCancerCells])))[2, 4]
  }
  rocOut <- calcSensitivitySpecificityFdr(top10pctEffectConventaionalModel, top10pctPVALConventaionalModel, simulatedEffectSizesCancer)
  sensitivitys[j] <- rocOut$Sensitivty
  specificitys[j] <- rocOut$Specificty
  trueFdrs[j] <- rocOut$MeasuredFalseDiscoveryRate
}
rocInteModel <- calcSensitivitySpecificityFdr(cancerEffectInteractionModel, pValuesCancerInteractionModel, simulatedEffectSizesCancer)

svg(file=paste(theRootDir, "paper/figures/suppFig_subsetting.svg", sep=""), height=3.5, width=7)
svg(file="/home/ubuntu/Dropbox/predixcanProj/paper/figures/suppfigs/suppFig_subsetting.svg", height=3.5, width=7)
plot(sensitivitys, type="l", ylim=c(0,1), bty="l", col="#386cb0", xaxt="n", xlab="Percentage of highest purity samples included", ylab="Proportion", las=1)
axis(1, at = 1:19, labels=(1:19*5), las=2)
lines(specificitys, col="#4daf4a")
lines(trueFdrs, col="#984ea3")
abline(h=rocInteModel$Sensitivty, lty=2, col="#386cb0")
abline(h=rocInteModel$Specificty, lty=2, col="#4daf4a")
abline(h=rocInteModel$MeasuredFalseDiscoveryRate, lty=2, col="#984ea3")
legend("right", inset=.05, title="", c("Specificity", "Sensitivity","FDR"), col=c("#4daf4a", "#386cb0", "#984ea3"), fill=c("#4daf4a", "#386cb0", "#984ea3"), cex=.8, bty="n")
dev.off()
